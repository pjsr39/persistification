...
"""
To run it:
>> python3 persistification_learning_case1.py --e_lower_int [THE_BEST_E_LOWER]
"""
...
csv_path = os.path.join(os.getcwd(),'results_with_learning_first_pair_DT.csv')
...
for n in range(NUM_POINTS + INITIAL_PART_DURATION):
    ...
    xdes2 = r * math.sin(m*timestep) + Xc2
    ydes2 = r * math.cos(m*timestep) * math.sin(m*timestep) + Yc2

    xdes2_dot = r * m * math.cos(m*timestep)
    ydes2_dot = r * m * math.cos(2*m*timestep)
    xdes2_ddot = -r * m**2 * math.sin(m*timestep)
    ydes2_ddot = -2 * r * m**2 * math.sin(2*m*timestep)
...
"""
"""(B1)
Learn canonical wave section. This is just a debug section.
"""
...
"""(C) 
Reset and initialize some new variables
"""
n = 0
x1d = 10
y1d = 6
...
for n in range(len(t)-1):
    if n % PRINT_PERIOD == 0:
        print(f"Time: {round(n * dt, 1)}, Dist: {round(arc_length, 3)}, Ed: {round(Ed,3)}, ncd: {needs_charging_d}, wncd: {wncd}, E: {round(E, 3)}, nci: {needs_charging_i}, vi: {vi}, vcs_c: {vcs_computed}, vi_c:{vi_computed}")
    timestep = n * dt
    logging.debug(f"iteration: {n}, time: {timestep}, E: {E}, Ed: {Ed}, arc_length: {arc_length}, vi: {vi}")

    # Circle around the charging station
    barrier_x = r_cs * math.cos(timestep) + x_cs
    barrier_y = r_cs * math.sin(timestep) + y_cs

    xdes1 = r * math.sin(c*timestep) + Xc1
    ydes1 = r * math.cos(c*timestep) * math.sin(c*timestep) + Yc1

    xdes2 = r * math.sin(m*timestep) + Xc2
    ydes2 = r * math.cos(m*timestep) * math.sin(m*timestep) + Yc2

    xdes1_dot = r * c * math.cos(c*timestep)
    ydes1_dot = r * c * math.cos(2*c*timestep)
    xdes1_ddot = -r * c**2 * math.sin(c*timestep)
    ydes1_ddot = -2 * r * c**2 * math.sin(2*c*timestep)
    ...
    with open(csv_path, mode='a', newline='') as file:
        ...
        writer.writerow([
            ...
            E, needs_charging_i, vi, arc_length, x1i, y1i, x1d, y1d
        ])
...
print(f'Distance covered: {str(arc_length)}, Idle time {len(t) * dt - t_dis}')

""" Writing to csv part """
df = pd.DataFrame(
    {'e_ia': ed_plot,
        'e_da': e_plot,
        'e_low_ia': e_lower_plot_d,
        'e_low_da': e_lower_plot_i,
        'e_min': e_min_plot,
        'e_ch': e_charge_plot,
    }
)
df.to_csv('first_pair.csv', index=False) # you change here the name
...
if args.animation:
    ...
    def init():
        ...
        return dumb_robot, time_text, intelligent_robot, circle1_line, circle2_line, circle3
    ...
    def animate(i):
        ...
        return dumb_robot, time_text, intelligent_robot, circle1_line, circle2_line, circle3, e_i_text, nci_text, e_d_text, ncd_text
