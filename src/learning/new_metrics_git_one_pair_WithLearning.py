...
# ===================== EQ. (2) AND EQ. (5) TERMS =====================
# These terms are included to reflect the article equations.
# Their effective contribution is set to zero so the original results are preserved.
...
for n in range(NUM_POINTS + INITIAL_PART_DURATION):
    ...
    # ===================== PV ENERGY DYNAMICS: Eq. (5), OFFLINE =====================
...
for n in range(len(t)-1):
    ...
    # ===================== PV ENERGY DYNAMICS: Eq. (5), MAIN LOOP =====================
    # Eq. (5): Edot_PV = -rho_PV(t) - phi(dij) - w_PV(t)

    d_pv_sv = math.sqrt((x1d - x1i)**2 + (y1d - y1i)**2)
    ...
    # ===================== SV ENERGY DYNAMICS: Eq. (2), MAIN LOOP =====================
    # Eq. (2): Edot_SV = -c1|v_SV|^2 - c2 - phi(dij) - w_SV(t)

    d_sv_pv = math.sqrt((x1i - x1d)**2 + (y1i - y1d)**2)
